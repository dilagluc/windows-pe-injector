...
int main(int ac, char **av)
{
    ...
    //encrypt 
    encrypt((PVOID)&__begin_of_code, nb_add);
    ...
    extern char __begin_decrypt;
    extern DWORD size;
    decrypt_code = (PUCHAR)decrypt - &__begin_of_code;
    VirtualProtect(size, sizeof(size), PAGE_READWRITE, &old_protect);
    size = nb_add;
    /*decrypt((PVOID)&__begin_of_code, nb_add);
    printf("DUMP DEENCRYPTED2::: %d\n", nb_add);
    for (int i=0; i<nb_add; i++)
        printf("%X ", ((PUCHAR)payload)[i]);
    printf("\n");*/
    //printf("ADD %X - %d\n", payload[0], nb_add);
