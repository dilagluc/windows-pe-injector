...
int injector(PVOID dll, char *thefile)
{
    ...
    //DWORD nb_add = *((&pDosHeader->e_res2[8]) - 0x2);
    ...
    DWORD size = pSection[idxSection].SizeOfRawData - (DWORD)((PUCHAR)relocSectionPtr - (PUCHAR)entryPoint);
    DWORD nb_add = size;
    ...
    my_memcpy_i(dstPtr, (PUCHAR)srcPtr, nb_add);
